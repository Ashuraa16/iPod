...
class App extends React.Component {
  constructor() {
    ...
    this.state = {
      ...
      songItems: ["Faded - Alan Walker", "Shape of You - Ed Sheeran", "Jai Shri Ram - Adipurush", "Hope - XXXTENTACION", "Rap God - Eminem"],  //song names
      songIndex: 0,  //current song
      lengthMenuKey: { "-1": 3, 1: 2, 4: 4, 8: 4, 3: 2, 9: 4, 10: 2 }, //length of particular menu
  ...
  render() {
    ...
    const { audio, active, currentMenu, menuItems, musicItems, songItems, playing, songIndex, theme, songUrl, songImgUrl, wheelColor, wallpaper, wallpapersItems, noty, notifyText } = this.state;

    return (
      <div className='background'>
        <div className='App'>
          <Case songIndex={songIndex} active={active} menuItems={menuItems} musicItems={musicItems} currentMenu={currentMenu} changeMenuForward={this.changeMenuForward} changeMenuBackward={this.changeMenuBackward} updateActiveMenu={this.updateActiveMenu} togglePlayPause={this.togglePlayPause} songItems={songItems} playing={playing} theme={theme} audio={audio} songUrl={songUrl} songImgUrl={songImgUrl} seekSongForward={this.seekSongForward} seekSongReverse={this.seekSongReverse} wheelColor={wheelColor} wallpaper={wallpaper} wallpapersItems={wallpapersItems} noty={noty} setNoty={this.setNoty} notifyText={notifyText} />
        </div>
      </div>
    );
  }
